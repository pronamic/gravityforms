 ...
 */
define( 'GF_MIN_WP_VERSION_SUPPORT_TERMS', '7.0' );
...
/**
 * Run the Jetpack Autoloader generated by Composer.
 */
require_once GF_PLUGIN_DIR_PATH . 'vendor/autoload_packages.php';
...
add_filter( 'user_has_cap', array( 'GFForms', 'user_has_cap' ), 10, 4 );
...
class GFForms {
	...
	*/
	...
	public static function loaded() {
		...
		require_once( GFCommon::get_base_path() . '/includes/abilities/class-gf-mcp-settings.php' );
		GF_MCP_Settings::init();
	...
	*/
	public static function register_services() {
		...
		if ( GF_SUPPORTS_ABILITIES_API ) {
			$container->add_provider( new \Gravity_Forms\Gravity_Forms\Abilities\GF_Abilities_Service_Provider() );
		}
	}
	...
	*/
	public static function get_service_container() {
		require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/class-gf-mcp-settings.php';

		if ( GF_SUPPORTS_ABILITIES_API && GF_MCP_Settings::is_enabled() && class_exists( '\WP\MCP\Core\McpAdapter' ) ) {
			\WP\MCP\Core\McpAdapter::instance();
		}

		if ( GF_SUPPORTS_ABILITIES_API ) {
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/class-gf-abilities-compatibility.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/class-gf-abilities-service-provider.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/schemas/class-gf-ability-schemas.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-forms.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-entries.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-submissions.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-feeds.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-notifications.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-confirmations.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-system.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/definitions/class-gf-abilities-definitions-notes.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-forms.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-entries.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-submissions.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-feeds.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-notifications.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-confirmations.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-notes.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/handlers/class-gf-abilities-handler-system.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/class-gf-abilities-registry.php';
			require_once GF_PLUGIN_DIR_PATH . 'includes/abilities/class-gf-ability.php';
		}
	...
	public static function init() {
		...
		add_action( 'current_screen', array( 'GFForms', 'localize_block_editor_assets' ) );
	...
	public static function initialize_admin_settings() {
		...
		if ( $gf_page === 'personal_data' ) {
			if ( ! class_exists( 'GF_Personal_Data' ) ) {
				require_once( GFCommon::get_base_path() . '/includes/personal-data/class-personal-data.php' );
			}
	...
	public static function no_conflict_mode_script() {
		...
		$gf_required_scripts = array(
			...
			'gf_export'                  => array( 'gform_form_admin', 'gform_field_filter', 'gform_gravityforms_admin' ),
	...
	/**
	 * Determines if the current page uses Gravity Forms components.
	 *
	 * Returns true if the current page is a Gravity Forms page or first-party add-on page that relies on Gravity Forms components.
	 *
	 * @since 3.1.1
	 *
	 * @return bool
	 */
	public static function use_gravity_forms_components() {
		$current_page   = self::get_page_query_arg();
		$gf_addon_pages = array( 'gravityflow-inbox', 'gravityflow-status', 'gravityflow-support', 'gravityflow-folders', 'gravityflow-checklists', 'gravityflow-reports', 'gravityflow-activity', 'gravityflow_settings' );

		/**
		 * Filters the add-on pages that use Gravity Forms components.
		 *
		 * @since 3.1.2
		 *
		 * @param string[] $gf_addon_pages The page slugs.
		 */
		$gf_addon_pages = gf_apply_filters( array( 'gform_addon_pages_using_components' ), $gf_addon_pages );

		return self::is_gravity_page() || in_array( $current_page, $gf_addon_pages );
	}
	...
	public static function get_status_messages( $plugin_name, $plugin_data, $slug = '', $version = '' ) {
		...
		if ( ( is_multisite() && ! is_network_admin() ) && version_compare( $version, rgar( $version_info, 'version' ), '<' ) ) {
			$changelog_nonce = wp_create_nonce( 'gf_get_changelog' );
			$changelog_url   = self_admin_url( 'admin-ajax.php?action=gf_get_changelog&plugin=' . $slug . '&_wpnonce=' . $changelog_nonce . '&TB_iframe=true&width=640&height=808' );
	...
	public static function register_scripts() {
		...
		wp_register_script( 'gform_field_filter', $base_url . "/js/gf_field_filter{$min}.js", array(), $version );
	...
	public static function maybe_prepend_hooks_js_script( $form_string ) {
		...
		// Don't prepend hooks js to the block editor page. In this context, hooks JS will be enqueued via localizing the gform_gravityforms script.
		if ( GFCommon::is_block_renderer_request() ) {
			return $form_string;
		}
	...
	public static function load_hooks_with_inline_script() {
		...
		// In the admin, gform_gravityforms_libraries is enqueued first, so we need to localize that script to ensure Hooks JS is loaded before it.
		$script_to_localize = is_admin() ? 'gform_gravityforms_libraries' : 'gform_gravityforms';

		wp_add_inline_script( $script_to_localize, $hooks_code, 'before' );
	}
	...
	public static function enqueue_admin_scripts( $hook ) {
		...
		if ( $page === 'form_editor' ) {
			...
			$form_strings = array(
				...
				'addButtonText'     => __( 'Add', 'gravityforms' ),
				'removeButtonText'  => __( 'Remove', 'gravityforms' ),
			);
	...
	public static function localize_block_editor_assets() {
		...
		if ( GFCommon::is_block_editor_page() ) {
			self::load_hooks_with_inline_script();
			wp_add_inline_script( 'gform_gravityforms', GFCommon::gf_global( false, false ) );
		}
	}
	...
	*/
	...
	public static function add_admin_body_class( $body_classes ) {
		...
		if ( self::use_gravity_forms_components() ) {
			$classes[] = 'gform-admin';
		}
...
if ( ! function_exists( 'gf_apply_filters' ) ) {
	 ...
	 *
	 * @since 1.9.12
	 * @since 1.9.15 Modifiers should no longer be passed as a separate parameter.
	 * @since 3.0.2  Updated to catch and log throwable errors.
	 *
	...
	function gf_apply_filters( $filter, $value ) {
		...
		try {
			...
			GFCommon::log_error( sprintf( '%s error from the %s filter callback in %s on line %d.', $e->getMessage(), $filter, $e->getFile(), $e->getLine() ) );
...
if ( ! function_exists( 'gf_do_action' ) ) {
	...
	function gf_do_action( $action ) {
		...
		try {
			// Apply modified versions of action
			foreach ( $modifiers as $modifier ) {
				$modifier = rgblank( $modifier ) ? '' : sprintf( '_%s', $modifier );
				$action  .= $modifier;
			...
			GFCommon::log_error( sprintf( '%s error from the %s action callback in %s on line %d.', $e->getMessage(), $action, $e->getFile(), $e->getLine() ) );
