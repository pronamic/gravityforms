...
namespace Gravity_Forms\Gravity_Forms\WP\MCP\Infrastructure\Observability;
 ...
 *
 * @package \Gravity_Forms\Gravity_Forms\WP\MCP\ObservabilityHandlers
 */
class ErrorLogMcpObservabilityHandler implements Contracts\McpObservabilityHandlerInterface
{
    ...
    public function record_event(string $event, array $tags = array(), ?float $duration_ms = null): void
    {
        ...
        if (null !== $duration_ms) {
            $log_message = sprintf('[MCP Observability] EVENT %s %.2fms %s', $formatted_event, $duration_ms, $formatted_tags);
        } else {
            $log_message = sprintf('[MCP Observability] EVENT %s %s', $formatted_event, $formatted_tags);
        }
    ...
    private static function format_tags(array $tags): string
    {
        ...
        $formatted = array_map(static function ($key, $value) {
