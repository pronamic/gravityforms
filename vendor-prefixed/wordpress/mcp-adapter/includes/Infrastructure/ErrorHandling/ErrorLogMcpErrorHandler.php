 ...
 *
 * @package \Gravity_Forms\Gravity_Forms\WP\MCP\ErrorHandlers
 */
class ErrorLogMcpErrorHandler implements Contracts\McpErrorHandlerInterface
{
    ...
    public function log(string $message, array $context = array(), string $type = 'error'): void
    {
        ...
        $log_message = sprintf('[%s] %s | Context: %s | User ID: %d', strtoupper($type), $message, wp_json_encode($context), $user_id);
