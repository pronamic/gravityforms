 ...
 *
 * @package \Gravity_Forms\Gravity_Forms\WP\MCP\Core
 */
declare (strict_types=1);
namespace Gravity_Forms\Gravity_Forms\WP\MCP\Core;

use Gravity_Forms\Gravity_Forms\WP\MCP\Abilities\DiscoverAbilitiesAbility;
use Gravity_Forms\Gravity_Forms\WP\MCP\Abilities\ExecuteAbilityAbility;
use Gravity_Forms\Gravity_Forms\WP\MCP\Abilities\GetAbilityInfoAbility;
use Gravity_Forms\Gravity_Forms\WP\MCP\Cli\McpCommand;
...
use Gravity_Forms\Gravity_Forms\WP\MCP\Servers\DefaultServerFactory;
use WP_Error;
...
final class McpAdapter
{
     ...
     * Registry instance
     *
     * @var \Gravity_Forms\Gravity_Forms\WP\MCP\Core\McpAdapter
     */
     ...
     *
     * @return \Gravity_Forms\Gravity_Forms\WP\MCP\Core\McpAdapter
     */
    ...
    private function register_wp_cli_commands(): void
    {
        ...
        \WP_CLI::add_command('mcp-adapter', McpCommand::class, array('shortdesc' => 'Manage MCP servers via WP-CLI.', 'longdesc' => 'Commands for managing and serving MCP servers, including STDIO transport.'));
    }
    /**
     ...
     * @param array<class-string<\Gravity_Forms\Gravity_Forms\WP\MCP\Transport\Contracts\McpTransportInterface>> $mcp_transports Array of MCP transport class names to initialize.
     * @param class-string<\Gravity_Forms\Gravity_Forms\WP\MCP\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface>|null $error_handler The error handler class name. If null, NullMcpErrorHandler will be used.
     * @param class-string<\Gravity_Forms\Gravity_Forms\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface>|null $observability_handler The observability handler class name. If null, NullMcpObservabilityHandler will be used.
     ...
     *
     * @return \Gravity_Forms\Gravity_Forms\WP\MCP\Core\McpAdapter|\WP_Error McpAdapter instance on success, WP_Error on failure.
     */
    public function create_server(string $server_id, string $server_route_namespace, string $server_route, string $server_name, string $server_description, string $server_version, array $mcp_transports, ?string $error_handler, ?string $observability_handler = null, array $tools = array(), array $resources = array(), array $prompts = array(), ?callable $transport_permission_callback = null)
    {
        ...
        if (!class_exists($error_handler)) {
            return new WP_Error('invalid_error_handler', sprintf(
        ...
        if (!class_exists($observability_handler)) {
            return new WP_Error('invalid_observability_handler', sprintf(
        ...
        try {
            $server = new McpServer($server_id, $server_route_namespace, $server_route, $server_name, $server_description, $server_version, $mcp_transports, $error_handler, $observability_handler, $tools, $resources, $prompts, $transport_permission_callback);
        } catch (\Throwable $e) {
            return new WP_Error('server_creation_failed', sprintf(
        ...
        $server->get_observability_handler()->record_event('mcp.server.created', array('status' => 'success', 'server_id' => $server_id, 'transport_count' => count($mcp_transports), 'tools_count' => count($tools), 'resources_count' => count($resources), 'prompts_count' => count($prompts)));
     ...
     *
     * @return \Gravity_Forms\Gravity_Forms\WP\MCP\Core\McpServer|null
     */
     ...
     *
     * @return \Gravity_Forms\Gravity_Forms\WP\MCP\Core\McpServer[]
     */
    ...
    {
        wp_register_ability_category('mcp-adapter', array('label' => 'MCP Adapter', 'description' => 'Abilities for the MCP Adapter'));
    }
    /**
