 ...
 */
class Gravity_Forms_Gravity_Forms_PHP_Autoloader {

	/**
	 * Registers the autoloader with PHP so that it can begin autoloading classes.
	 *
	 * @param Gravity_Forms_Gravity_Forms_Version_Loader $version_loader The class loader to use in the autoloader.
	 */
	...
	public function unregister_autoloader() {
		...
		foreach ( $autoload_chain as $autoloader ) {
			...
			if ( 'Gravity_Forms\\Gravity_Forms\\Automattic\\Jetpack\\Autoloader\\jp' === substr( $namespace_check, 0, 32 ) ) {
	...
	public static function load_class( $class_name ) {
		...
		if ( defined( 'JETPACK_AUTOLOAD_DEBUG_EARLY_LOADS' ) && JETPACK_AUTOLOAD_DEBUG_EARLY_LOADS &&
			...
			$msg = "Jetpack Gravity_Forms_Gravity_Forms_Autoloader: Autoloading `$class_name` before the plugins_loaded hook may cause strange and confusing problems. " . wp_debug_backtrace_summary( '', 1 );
