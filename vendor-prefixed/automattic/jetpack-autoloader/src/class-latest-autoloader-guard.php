 ...
 */
class Gravity_Forms_Gravity_Forms_Latest_Autoloader_Guard {

	/**
	 * The Gravity_Forms_Gravity_Forms_Plugins_Handler instance.
	 *
	 * @var Gravity_Forms_Gravity_Forms_Plugins_Handler
	 */
	private $plugins_handler;

	/**
	 * The Gravity_Forms_Gravity_Forms_Autoloader_Handler instance.
	 *
	 * @var Gravity_Forms_Gravity_Forms_Autoloader_Handler
	 */
	 ...
	 *
	 * @var Gravity_Forms_Gravity_Forms_Autoloader_Locator
	 */
	 ...
	 * The constructor.
	 *
	 * @param Gravity_Forms_Gravity_Forms_Plugins_Handler    $plugins_handler    The Gravity_Forms_Gravity_Forms_Plugins_Handler instance.
	 * @param Gravity_Forms_Gravity_Forms_Autoloader_Handler $autoloader_handler The Gravity_Forms_Gravity_Forms_Autoloader_Handler instance.
	 * @param Gravity_Forms_Gravity_Forms_Autoloader_Locator $autoloader_locator The Gravity_Forms_Gravity_Forms_Autoloader_Locator instance.
	 */
			...
			if ( is_array( $autoloader ) && $autoloader[0] instanceof \Composer\Autoload\ClassLoader && is_callable( array( $autoloader[0], 'getPrefixesPsr4' ) ) ) {
				...
				foreach ( $composer_autoloader->getClassMap() as $classname => $path ) {
					if ( $jetpack_autoloader_loader->find_class_file( $classname ) ) {
						$msg = "A Composer autoloader is registered with a higher priority than the Jetpack Gravity_Forms_Gravity_Forms_Autoloader and would also handle some of the classes we handle (e.g. $classname => $path). This may cause strange and confusing problems.";
				...
				foreach ( $composer_autoloader->getPrefixesPsr4() as $prefix => $paths ) {
					if ( isset( $prefixes[ $prefix ] ) ) {
						...
						$msg  = "A Composer autoloader is registered with a higher priority than the Jetpack Gravity_Forms_Gravity_Forms_Autoloader and would also handle some of the namespaces we handle (e.g. $prefix => $path). This may cause strange and confusing problems.";
						wp_trigger_error( '', $msg );
						continue 2;
					}
				}
