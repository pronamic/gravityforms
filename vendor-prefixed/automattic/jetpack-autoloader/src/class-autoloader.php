 ...
 */
class Gravity_Forms_Gravity_Forms_Autoloader {

	/**
	 * Checks to see whether or not the autoloader should be initialized and then initializes it if so.
	 *
	 * @param Gravity_Forms_Gravity_Forms_Container|null $container The container we want to use for autoloader initialization. If none is given
	...
	public static function init( $container = null ) {
		...
		if ( ! isset( $container ) ) {
			...
			$container = new Gravity_Forms_Gravity_Forms_Container();
		}

		// phpcs:disable Generic.Commenting.DocComment.MissingShort

		/** @var Gravity_Forms_Gravity_Forms_Autoloader_Handler $autoloader_handler */
		$autoloader_handler = $container->get( Gravity_Forms_Gravity_Forms_Autoloader_Handler::class );
		...
		/** @var Gravity_Forms_Gravity_Forms_Plugin_Locator $plugin_locator */
		$plugin_locator = $container->get( Gravity_Forms_Gravity_Forms_Plugin_Locator::class );

		/** @var Gravity_Forms_Gravity_Forms_Plugins_Handler $plugins_handler */
		$plugins_handler = $container->get( Gravity_Forms_Gravity_Forms_Plugins_Handler::class );
		...
		/** @var Gravity_Forms_Gravity_Forms_Latest_Autoloader_Guard $guard */
		$guard = $container->get( Gravity_Forms_Gravity_Forms_Latest_Autoloader_Guard::class );
		...
		/** @var Gravity_Forms_Gravity_Forms_Hook_Manager $hook_manager */
		$hook_manager = $container->get( Gravity_Forms_Gravity_Forms_Hook_Manager::class );

		// Register a shutdown handler to clean up the autoloader.
		$hook_manager->add_action( 'shutdown', new Gravity_Forms_Gravity_Forms_Shutdown_Handler( $plugins_handler, $cached_plugins, $was_included_by_autoloader ) );
